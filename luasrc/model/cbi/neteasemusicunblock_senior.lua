...
mp = Map("neteasemusicunblock", translate("解除网易云音乐播放限制 (Golang)"))
mp.description = translate("原理：采用 [酷我/酷狗/咕咪] 音源(后续有空补充)，替换网易云音乐 灰色 歌曲链接<br/>具体使用方法参见：https://github.com/siwind/luci-app-neteasemusicunblock<br/>提示：客户端网易云音乐能用就别升级app，最新版本不一定能用")
mp:section(SimpleSection).template = "neteasemusicunblock/neteasemusicunblock_status"

s = mp:section(TypedSection, "neteasemusicunblock")
s.anonymous=true
s.addremove=false
...
local currentTagCMD="neteasemusicunblock -v |grep Version|awk '{print $2}'"
local currentRuntimeCMD="neteasemusicunblock -v |grep runtime|awk -F\\( '{print $2}'|awk '{print $3,$4}'|sed -E 's/\)//g'|sed 's/[ \t]*$//g'"
...
function download_()
	local sFile, block
	sFile=nixio.open("/usr/share/neteasemusicunblock/ca.crt","r")
...
function delete_()
	local sPath, fd
	sPath = "/usr/share/neteasemusicunblock/server.crt"
	fd = os.remove(sPath)
    if not fd then
